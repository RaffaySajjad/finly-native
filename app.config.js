...
module.exports = ({ config }) => {
  ...
  if (!apiUrl) {
    console.warn(
      `[app.config.js] EXPO_PUBLIC_API_URL not set for environment: ${env}`
    );
  } else {
    console.log(`[app.config.js] Using API URL for ${env}: ${apiUrl}`);
    if (
      atsHostname &&
      atsHostname !== 'localhost' &&
      !atsHostname.includes('.')
    ) {
      console.warn(
        `[app.config.js] Warning: Could not extract hostname from API URL: ${apiUrl}`
      );
    }
  }
  ...
  // Preserve EAS projectId from config or use the one from app.json
  const existingEas = existingExtra.eas || {
    projectId: 'dfa0ffd0-4dca-4765-b6dd-1ca535a8e731'
  };
