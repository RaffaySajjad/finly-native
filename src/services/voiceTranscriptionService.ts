...
export async function transcribeAudio(
  ...
  try {
    ...
    // Determine MIME type
    const mimeType = ext === 'm4a' ? 'audio/m4a' : ext === 'mp3' ? 'audio/mpeg' : ext === 'wav' ? 'audio/wav' : 'audio/m4a';
    ...
    // Build query params
    const queryParams = languageCode ? `?languageCode=${encodeURIComponent(languageCode)}` : '';

    console.log('[VoiceTranscription] Uploading audio file:', filename);

    // Make request with FormData
    // Note: Content-Type will be automatically removed by apiClient interceptor for FormData
    const response = await apiClient.post<{ success: boolean; data: { text: string } }>(
    ...
    if (response.data?.success && response.data?.data?.text) {
      ...
      console.log('[VoiceTranscription] Transcription successful:', transcribedText.substring(0, 50) + '...');
      return transcribedText;
    }

    console.warn('[VoiceTranscription] Unexpected response format:', response.data);
    return null;
  } catch (error: any) {
    ...
    if (error.response?.status === 400) {
      throw new Error(error.response.data?.error?.message || 'Invalid audio file format.');
    }
